...
int set_opt(jint nBits, jchar nEvent, jint nStop) {
    ...
    LOGE("set_opt:nBits=%d,nEvent=%c,nSpeed=%d,nStop=%d", nBits, nEvent, nStop, nStop);
    ...
    switch (2) {
        case 0:
            newtio.c_cflag &= ~CRTSCTS;    //不使用流控
            break;
        case 1:
            newtio.c_cflag |= CRTSCTS;    //硬件流控
            break;
        case 2:
            newtio.c_cflag |= IXON | IXOFF | IXANY;    //软件流控
            break;
        default:
    ...
    newtio.c_cc[VTIME] = (cc_t) 400;//设置等待时间
...
Java_com_lksoft_jin_serialporttool_SerialPort_open__Ljava_lang_String_2II(JNIEnv *env, jobject type,
                                                                          jstring path,
                                                                          jint baudrate,
                                                                          jint flags) {
    int fd;
...
JNIEXPORT jobject JNICALL
Java_com_lksoft_jin_serialporttool_SerialPort_open__Ljava_lang_String_2IIIC(JNIEnv *env,
                                                                            jobject type,
                                                                            jstring path,
                                                                            jint baudrate,
                                                                            jint databits,
...
Java_com_lksoft_jin_serialporttool_SerialPort_close(JNIEnv *env, jobject instance) {
